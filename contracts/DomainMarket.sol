...
contract DomainMarket is ERC721URIStorage {
    ...
    modifier validateLength(string calldata name) {
        if (!(StringUtils.strlen(name) >= 3 && StringUtils.strlen(name) <= 6)) revert InvalidName(name);
        _;
    }
        ...
        validateLength(name)
    {
        ...
        string memory json = Base64.encode(
            abi.encodePacked(
                '{"name": "',
                _name,
                '", "description": "A domain on the Music name service", "image": "data:image/svg+xml;base64,',
